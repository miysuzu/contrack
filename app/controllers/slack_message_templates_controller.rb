class SlackMessageTemplatesController < ApplicationController
  before_action :authenticate_user!
  before_action :set_template, only: [:show, :edit, :update, :destroy]

  def index
    @templates = SlackMessageTemplate.where(user: current_user).order(:category, :name)
  ...
  def destroy
    @template.destroy
    redirect_to slack_message_templates_path, notice: 'テンプレートを削除しました。'
  end
  ...
  def set_template
    @template = SlackMessageTemplate.where(user: current_user).find(params[:id])
  end
